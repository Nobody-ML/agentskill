...
- 门禁状态机：`protocols/00-hard-gates.md`（G6/G7/G8）
- 执行预检：`protocols/03-execution-preflight.md`
- 兜底与边界：`protocols/07-fallback-and-boundaries.md`
- 长任务运行与监控（Ops/训练/长实验）：`protocols/08-long-running-ops.md`
- 长程恢复（上下文压缩/漂移）：`protocols/05-resumption-and-anti-drift.md`

常用模板：
- `templates/ExecutionPreflight.template.md`
- `templates/Ops-Runbook.template.md`（WorkType=Ops/长任务）
...
- 通用工程作战模型（Build/Debug/Ops）：`library/software-engineering-operating-model.md`
- Task 拆分标准（任务组与验证节奏）：`library/task-decomposition-standard.md`
...
- 长任务续跑与 Ops 作战手册：`library/long-run-agent-operations.md`
...
- 执行前预检摘要（建议按 `templates/ExecutionPreflight.template.md` 落盘）
- 勾选了哪些小任务（或标记为进行中）
