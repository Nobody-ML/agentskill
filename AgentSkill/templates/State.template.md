...
### 3.6 Resumption Block（上下文压缩/交接恢复必备）

> 要求：短、可执行、可复查。用于上下文压缩后恢复，不是“总结作文”。

（建议直接贴入并维护一份 YAML；格式参考：`templates/ResumptionBlock.template.md`）

### 3.7 Gate Summary（门禁摘要，建议维护）

记录最近一次关键门禁结果（G0–G9），用于快速判断是否允许进入执行/交付。
...
gates:
  ...
  G4: blocked
  G7: pass
last_gate_update: YYYY-MM-DD HH:MM
```
